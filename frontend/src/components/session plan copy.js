...
const SessionPlans = () => {
  ...
  const handleGenerateLessonPlan = async (sessionNumber, topicIndex) => {
    try {
      ...
      const payload = {
        ...
        unit: unitName,
        chapter: topic.name,
        topics: topic.concepts.map((concept, index) => ({
          topic: topic.name,
          concept,
          conceptDetailing: topic.conceptDetailing.split(";")[index] || "",
        })), // Include concept detailing
        sessionType: "Theory",
        noOfSession: 1,
        duration: 45,
      };
      ...
      setError(
        `Failed to generate lesson plan for topic "${topicsWithConcepts[sessionNumber][topicIndex]?.name}".`
      );
    }
  };
  ...
  const handleDownloadLessonPlan = async (
    ...
    try {
      ...
      const fileName = `${topicName}-${conceptName || "General"}.pdf`;
      ...
      console.error("Error downloading lesson plan as PDF:", error);
      setError("Failed to download lesson plan as PDF.");
    }
  };
