          ...
          {periods.map((period, index) => {
            ...
            return (
              <React.Fragment key={index}>
                <tr>
                  <td>{periodName} <br /> {periodTime}</td>
                  {days.map(day => {
                    ...
                    return (
                      <td key={`${day}-${period}`} onClick={() => !isReservedWithinPeriod && handleOpenModal(day, period)}>
                        {isReservedWithinPeriod ? (
                          <span className="reserved">Reserved Time ({reserveStart} - {reserveEnd})</span>
                        ) : periodAssignment ? (
                          <>
          ...
          <tr>
            ...
            {days.map(day => {
              ...
              return (
                <td key={day}>
                  {isAfterSchoolHours ? (
                    <div className="reserved">afterschool hours <br /> {`${commonReserveStart} to ${commonReserveEnd}`}</div>
                  ) : reserveAfterSchool ? (
                    <div className="reserved">afterschool hours <br /> {`${reserveDay[day].start} to ${reserveDay[day].end}`}</div>
                  ) : (
                    <span>-</span>
                  )}
                </td>
